...
the real notes.
  ...
  python scripts/persona_synthesize.py <hub> <cache_dir> <persona> <topic> [--final]
"""
...
def main(hub: Path, cache_dir: Path, persona: str, topic: str, final: bool) -> int:
    real_root = hub / "wiki" / "personas" / persona
    llm = CacheLLM(cache_dir / topic)

    if final:
        root = real_root
    else:                       # warm-up: never write into the real vault
        tmp = Path(tempfile.mkdtemp(prefix=f"{persona}-{topic}-"))
        (tmp / "raw").symlink_to(real_root / "raw")
        root = tmp

    try:
        res = synthesize(root, topic, llm, date.today().isoformat())
...
if __name__ == "__main__":
    ...
    sys.exit(main(Path(args[0]), Path(args[1]), args[2], args[3], "--final" in sys.argv))
