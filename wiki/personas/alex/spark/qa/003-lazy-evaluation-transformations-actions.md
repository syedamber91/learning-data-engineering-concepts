...
qc: passed
slug: 003-lazy-evaluation-transformations-actions
topics:
- spark
learner: alex
...
**Alex:** You said calling `.cache()` by itself doesn't store anything until an action runs — so if I call `.cache()` on an RDD and then genuinely never call any action on it, ever, does that RDD just sit there uncached forever, like the `.cache()` call did literally nothing at all?
