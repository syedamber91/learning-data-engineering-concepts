...
Vu flags, without elaborating further, that this checkpoint process is also what underpins Flink's exactly-once delivery guarantees — a connection his sources point to via an external Flink blog post rather than explain directly. What actually composes with the checkpoint barrier to deliver exactly-once semantics (sink-side idempotence is one plausible mechanism, but the source never names it) is left as a **source gap**.
