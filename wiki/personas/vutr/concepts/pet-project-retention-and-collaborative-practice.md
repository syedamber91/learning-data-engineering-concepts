...
His corrective is a short list of his own practices, not a citation to anyone else's framework. Before starting, set explicit expectations for what the project is meant to teach. While building, "do things right" rather than just get them working — learn the fundamentals and best practices of Docker, Git, and other tools, organize and test code properly, and understand mechanics like SQL's order of execution. He also recommends doing the project with friends specifically to learn what building alongside more than one person is like, since a solo project can't teach that. For every tool touched — Spark, Airflow, Git, Docker — his suggested habit is to ask two questions: "what problem do these tools try to solve?" and "how are they gonna solve that problem?" rather than only learning the tool's surface commands. After the project runs, he suggests role-playing as the eventual consumer of the data warehouse or dashboard and honestly asking "Am I satisfied? Do I get things I need?" — grounded in his claim that a data engineer's job is, most of the time, serving that internal data-consuming user. Finally, he names applying data modeling to the project.
