...
The paper frames fault tolerance itself as tunable per application rather than uniform across the platform: Puma provides fault tolerance for stateful aggregation via its HBase checkpoints, while Stylus offers multiple fault-tolerant options precisely because its applications' latency and consistency needs vary more widely.
