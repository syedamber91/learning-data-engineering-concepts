...
**Spatial partitioning** instead divides the vector space into partitions, each represented by a point (typically the centroid of the vectors it contains). A query first finds the closest representative centroid, then loads only the partition(s) associated with it to search for matches, rather than touching the whole dataset.
