...
These aren't abstract values — they show up directly in the choices [[airbnb-gold-silver-hadoop-clusters]] describes: Airflow, Hive, Presto, and Spark are all adopted open-source components rather than bespoke builds; the Gold/Silver split and the later Cloudera partnership are "standard components" bets (paying for Cloudera's expertise rather than reinventing cluster monitoring); the Mesos-to-dedicated-instances migration and the EC2-instance-type-per-workload change are scalability fixes; and Airpal — a purpose-built web query tool on top of Presto — is a direct answer to what internal users needed to run SQL day to day.
