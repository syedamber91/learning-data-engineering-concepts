...
This is what lets segments transition cleanly from Helix's `CONSUMING` state to `ONLINE` during real-time ingestion (see [[pinot-cluster-components]]) — the protocol is the missing piece between "servers are independently reading a Kafka topic" and "the resulting immutable segment is the same no matter which replica produced it" (see [[immutable-segment]]).
