...
The mechanism, per the notes: dbt's incremental model creates a temporary table named `destination_table__dbt_tmp`, uses it to update the destination table incrementally, and deletes it once the update finishes. That naming scheme assumes one run per destination table at a time. When Discord tried to run multiple incremental updates against the same table in parallel — each covering a different data range — every concurrent run created a temp table with the *same* `__dbt_tmp` name, so one run's in-flight temp table could be deleted or overwritten by another run still using it, causing conflicts.
