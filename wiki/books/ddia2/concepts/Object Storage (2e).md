---
book: Designing Data-Intensive Applications (2nd Edition)
edition: 2
type: concept
tags: [ddia2, concept, cloud, storage, s3]
sources:
  - raw/ch01.md
  - raw/ch11.md
---
# Object Storage

Durable, effectively unbounded blob storage addressed by key, with no filesystem semantics. In the 2nd edition it has largely displaced HDFS as the substrate for batch processing and increasingly backs log brokers and warehouses too.

See [[Distributed Filesystems (2e)]] and [[Separation of Storage and Compute (2e)]].

## Appears In
- [[Analytics (2e)]]
- [[Batch Processing in Distributed Systems (2e)]]
- [[Batch and Stream Processing (2e)]]
- [[Cloud Data Warehouses (2e)]]
- [[Cloud Native System Architecture (2e)]]
- [[Column-Oriented Storage (2e)]]
- [[Data Storage for Analytics (2e)]]
- [[Data Systems, Law, and Society (2e)]]
- [[Data Warehousing (2e)]]
- [[Dataflow Engines (2e)]]
- [[Distributed Filesystems (2e)]]
- [[Distributed Job Orchestration (2e)]]
- [[Distributed Locks and Leases (2e)]]
- [[Distributed Versus Single-Node Systems (2e)]]

## In the vutr data-engineering wiki
- [[amazon-s3-gfs-hdfs-and-distributed-file-systems]] — the GFS→HDFS→S3 lineage behind the 2nd edition's shift away from distributed filesystems.
